		...
		btnguardar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					...
					String nacimiento = String.valueOf(LocalDate.parse(formatter.format(txtFechaN.getDate())));
					String nacionalidad = (String) txtNacionalidad.getSelectedItem();
					String telefono = txtTelefono.getText();
					Integer reservaId = Integer.valueOf(txtNreserva.getText());

					
					boolean dataIsValit = (nombre.length() > 0)  
										&& apellido.length() > 0 
										&& nacimiento.length() > 0 
					...
					System.out.println(dataIsValit);
					if (dataIsValit) {
						Huespedes huesped = new Huespedes(nombre, apellido, nacimiento, nacionalidad, telefono,
								reservaId);

						// guardando datos del huésped.
						...
						JOptionPane.showMessageDialog(null, "No se permite enviar campos vacios/incompletos.");
					}

				} catch (Exception e2) {
					
					JOptionPane.showMessageDialog(null, "Error al enviar registro.");
				}
			}
